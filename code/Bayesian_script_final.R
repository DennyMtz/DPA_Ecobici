...
#		Datos
#	----------------------------------------------------
#dde <- read.csv(paste('master_data.csv',sep = ""), header = TRUE, sep = ",", quote="\"", dec=".", fill = TRUE)
dde <- read.csv(paste('master_y.csv',sep = ""), header = TRUE, sep = ",", quote="\"", dec=".", fill = TRUE)

# dde$yy <- seq(0.0,length(nrow(dde$flujo_rel)))
...
# }
# 
# write.csv(dde,file='../data/master_y.csv')

# 0 equivale a que están balanceadas (376,107 obs)
# equivale que están desvaleanceadas (314,499 obs)


ggplot(dde, aes(x=flujo_rel)) +
  geom_histogram(breaks=seq(-1, 1, by=.01), colour="black",  fill="black")
...
corr_data <-cbind(dde_1$edad_prom, dde_1$prop_fem, dde_1$duracion_prom, dde_1$distancia_prom, dde_1$maxcapacity, dde_1$MAX_DIA_O3, dde_1$MAX_DIA_PM10, dde_1$MAX_DIA_RH, dde_1$MAX_DIA_WDR, dde_1$MAX_DIA_WSP)
table_corr <- cor(corr_data)
write.csv(table_corr,file='../data/tabla_correlaciones.csv')
#EN ESTE ANALISIS OBSERVAMOS QUE LA DURACION Y LA DISTANCIA ESTAN ALTAMENTE CORRELACIONADAS CON UN 82%, ASIMISMO LA EDAD PROMEDIO CON LA DURACION
#PROMEDIO Y CON LA DISTANCIA PROMEDIO.
...
#	--------------------------------------------			
#REGION 0
X_r0 <- subset(dde_1, dde_1$region_0==1)
...
for(gt in 1:G){
  ...
  #	Output
  write(t(beta),file=paste(path_to_project,'results/R_0_beta.out',sep = ""), ncol=ncol(X), append=T)
  print(c(gt,round(beta*100)/100))
}

#	Trace-plots para beta[1] y beta[2]
#pdf(paste(path_to_project,'results/R_0_beta_traces.pdf',sep = ""),width=7,height=5)
# par(mfrow=c(2,1))
beta_out<- matrix(scan(paste(path_to_project,'results/R_0_beta.out',sep = "")), ncol=ncol(X), byrow=T)
# plot(beta_out[,1],type="l",xlab="iteration",ylab="(beta_1)")
# plot(beta_out[,2],type="l",xlab="iteration",ylab="(beta_2)")
#dev.off()

pdf(paste(path_to_project,'results/R_0_beta_traces_0.pdf',sep = ""),width=7,height=5)
par(mfrow=c(3,3))
...
pdf(paste(path_to_project,'results/R_0_beta_traces_1.pdf',sep = ""),width=7,height=5)
par(mfrow=c(3,3))
...
pdf(paste(path_to_project,'results/R_0_beta_traces_2.pdf',sep = ""),width=7,height=5)
par(mfrow=c(3,3))
...
print(betas_ok)
write.csv(betas_ok,file='../results/R_0_betas_finales.csv')

#	Distribucion final marginal
pdf(paste(path_to_project,'results/R0_density_1.pdf',sep = ""),width=7,height=5)
par(mfrow=c(3,3))
...
pdf(paste(path_to_project,'results/R0_density_2.pdf',sep = ""),width=7,height=5)
par(mfrow=c(3,3))
...
pdf(paste(path_to_project,'results/R0_density_3.pdf',sep = ""),width=7,height=5)
par(mfrow=c(3,3))
...
#	Curva de respuesta a dosis DDE para nacimiento prematuro ## HAY QUE ADAPTARLO A NUESTRO PROBLEMA, PUSE DURACION PROMEDIO
# pdf(paste(path_to_project,'results/R0_beta_doseresponse.pdf',sep = ""),width=7,height=5)
# par(mfrow=c(1,1))
# par(mar=c(5,5,5,5))
...
# }
# xtrue<- xg*sqrt(var(dde_1$duracion_prom)) + mean(dde_1$duracion_prom) #hay que adaptarlo
